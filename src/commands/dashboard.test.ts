...
import { parseDashboardFlags, runDashboard, type DashboardRunOptions } from './dashboard.js';
...
function baseOpts(overrides: Partial<DashboardRunOptions> = {}): DashboardRunOptions {
  return {
...
describe('runDashboard', () => {
  ...
  it('on port conflict, tries the next port and prints the URL', async () => {
    ...
    try {
      ...
      expect(result.url).toMatch(new RegExp(`http://127\\.0\\.0\\.1:${result.port}/`));
  ...
  it('does not call fix apply from the dashboard command path', async () => {
    ...
    expect(codeOnly).not.toMatch(/\bapplyFix\b|\bapplyFixPlan\b|\brunFix\b|\bfixApply\b/);
