...
import { parseStatusFlags, runStatus, scopeFromFlags } from './status.js';
...
describe('runStatus', () => {
  ...
  it('status --all walks all mapped projects (no count cap)', async () => {
    ...
    expect(report.scope).toBe('machine');
    const missing = report.findings.filter((f) => f.id === 'instructions.missing_file');
    expect(missing.some((f) => f.evidence.some((e) => e.includes(projA)))).toBe(true);
    expect(missing.some((f) => f.evidence.some((e) => e.includes(projB)))).toBe(true);
  });
  ...
  it('prints overall grade and sync matrix in terminal mode', async () => {
    ...
    const { report, exitCode } = await runStatus({
      ...
      checks: {
        map: baseMap({ global_roots: [hub], sync_target: hub }),
        adapters: [stubAdapter('claude-code', [hub]), stubAdapter('codex', [privateRoot])],
      },
      stdout: (line) => lines.push(line),
    });
  ...
  it('process.exitCode matches grade mapping', async () => {
    ...
    expect(process.exitCode).toBe(exitCodeForGrade(green.report.overall.grade));
    ...
    expect(process.exitCode).toBe(exitCodeForGrade(desync.report.overall.grade));
