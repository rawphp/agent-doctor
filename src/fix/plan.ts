...
function appendActionsFromFindings(findings: Finding[]): FixAction[] {
  ...
  for (const finding of findings) {
    ...
    const instructions = evidence.filter(
      (p) => p !== product && /\.md$/i.test(p),
    );
    const agents =
      finding.agents_affected.length > 0
        ? finding.agents_affected
        : [undefined];
          ...
          out.push({
            id:
              instructions.length === 1
                ? id
                : `${id}__${instr.split(/[/\\]/).pop()}`,
            kind: "append_instruction_link",
            description:
              instructions.length === 1
                ? description
                : `Append link to ${basename} in ${instr}`,
            target: instr,
            value: product,
            agent_id: agentId,
 ...
 */
function buildFixPlanFromReport(
  report: Report,
  ...
  for (const action of report.fix_plan ?? []) {
    ...
    if (
      !SAFE_FIX_KINDS.has(action.kind) &&
      !WIRE_KINDS.has(action.kind)
    ) {
  ...
  const existingAppend = new Set(
    plan
      .filter(
        (a) =>
          a.kind === "append_instruction_link" ||
          a.kind === "append_link_block",
      )
...
function isBuildFixPlanInput(
  value: Report | BuildFixPlanInput,
): value is BuildFixPlanInput {
  return (
    value != null &&
    typeof value === "object" &&
