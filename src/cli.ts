...
function printCommandHelp(command: CommandName): void {
  ...
  const bodies: Record<CommandName, string[]> = {
    ...
    map: [
      ...
      '',
      'Vault options (path must exist; recorded as source: manual):',
      '  --vault <path>            Add vault (default) — keeps existing vaults',
      '  --add-vault <path>        Same as --vault (explicit add)',
      '  --set-vault <path>        Replace: this path becomes the only vault',
      '  --vault <path> --replace  Same as --set-vault',
      '  --yes, --non-interactive  Skip prompts',
      '',
      'Examples:',
      '  agent-doctor map --add-vault ~/Notes',
      '  agent-doctor map --set-vault ~/EA/cowork/meaning-of-life',
      '  agent-doctor map --vault ~/Work --replace',
    ],
    status: [
    ...
    fix: [
      ...
      '  --force                 Replace non-empty agent skills dirs with hub symlinks',
      '  --sync-target <path>    Choose skills hub when multiple roots conflict',
      '',
      'Examples:',
      '  agent-doctor fix --dry-run --sync-target ~/.agents/skills --html',
      '  agent-doctor fix --yes --force   # after merging unique skills into the hub',
    ],
    agents: [
    ...
    check: [
      ...
      '',
      'Instruction hierarchy findings (project/hybrid; also in status --json):',
      '  instructions.hierarchy_missing_agents_md   Project root lacks AGENTS.md',
      '  instructions.hierarchy_missing_pointer     Vendor file missing AGENTS.md pointer',
      '',
      'Options:',
