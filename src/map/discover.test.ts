...
describe('discover', () => {
  ...
  it('populates multiple candidate skills roots without a single hard-coded hub', () => {
    ...
    // No single hero path is treated as the only possible hub — all existing
    // candidates are returned as peers.
  ...
  it('discovers project root candidates under common locations', () => {
    ...
    expect(result.project_roots).toEqual(expect.arrayContaining([projects, developer]));
  });
  ...
  it('discovers vault candidates via .obsidian markers in common locations', () => {
    ...
    // decoy without .obsidian should not count
    ...
    expect(result.vaults.map((v) => v.path)).toEqual(expect.arrayContaining([vaultA, vaultB]));
