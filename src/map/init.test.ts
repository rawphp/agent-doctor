...
describe('init / map path-unit', () => {
  ...
  it('init writes versioned map.yml under agent-doctor config home', async () => {
    ...
    expect(loaded!.skills.global_roots).toContain(join(fixtureHome, '.agents', 'skills'));
  ...
  it('detects Claude Code, Codex, and Grok homes when present', async () => {
    ...
    expect(map.agents.find((a) => a.id === 'codex')?.config_home).toBe(join(fixtureHome, '.codex'));
    expect(map.agents.find((a) => a.id === 'grok')?.config_home).toBe(join(fixtureHome, '.grok'));
  });
  ...
  it('map refreshes discovery without vault prompt (no wizard chrome)', async () => {
    ...
    expect(promptVault).not.toHaveBeenCalled();
    expect(map.agents.map((a) => a.id).sort()).toEqual(['claude-code', 'grok']);
    expect(map.skills.global_roots).toContain(join(fixtureHome, '.agents', 'skills'));
  });
