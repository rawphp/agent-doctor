...
function finding(partial: Pick<Finding, 'id'> & Partial<Finding>): Finding {
  return {
    severity: 'error',
    domain: 'skills',
...
describe('capGradeForDesync', () => {
  it('does not change grade when aligned and no hub conflict', () => {
    expect(capGradeForDesync('green', { aligned: true, hubConflict: false })).toBe('green');
  });

  it('never allows green when skills desync (aligned false)', () => {
    expect(capGradeForDesync('green', { aligned: false, hubConflict: false })).toBe('yellow');
  });

  it('never allows green on unresolved multi-hub conflict', () => {
    expect(capGradeForDesync('green', { aligned: true, hubConflict: true })).toBe('yellow');
  });

  it('does not raise yellow or red on desync', () => {
    expect(capGradeForDesync('yellow', { aligned: false, hubConflict: false })).toBe('yellow');
    expect(capGradeForDesync('red', { aligned: false, hubConflict: true })).toBe('red');
  });
});

describe('findingsBlockGreen / DESYNC_FINDING_IDS', () => {
  it('documents desync finding ids that cap overall grade', () => {
    expect([...DESYNC_FINDING_IDS]).toEqual(['skills.agent_not_on_hub', 'skills.hub_conflict']);
  });

  it('returns true for skills.agent_not_on_hub (non-ignored first-class off hub)', () => {
    expect(
      findingsBlockGreen([finding({ id: 'skills.agent_not_on_hub', agents_affected: ['claude'] })]),
    ).toBe(true);
  });

  it('returns true for skills.hub_conflict', () => {
    expect(findingsBlockGreen([finding({ id: 'skills.hub_conflict' })])).toBe(true);
  });
