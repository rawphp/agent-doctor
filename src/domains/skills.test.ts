...
describe("checkSkills", () => {
  ...
  it("flags agents not on the hub", async () => {
    ...
    expect(offHub.every((f) => !f.agents_affected.includes("claude-code"))).toBe(
      true,
    );
  });
  ...
  it("flags duplicated skill trees across agent homes", async () => {
    ...
    const { findings } = await checkSkills({
      ...
      adapters: [
        stubAdapter("claude-code", [claudeSkills]),
        stubAdapter("codex", [codexSkills]),
      ],
    });
  ...
  it("when hub is missing, does not invent on-hub status", async () => {
    ...
    expect(
      findings.every((f) => f.id !== "skills.agent_not_on_hub" || f.sync_target),
    ).toBe(true);
  });
});
