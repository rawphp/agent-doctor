...
function mapWithVaults(
  vaults: HomeMap["vaults"],
): HomeMap {
  return {
    version: 1,
...
describe("checkObsidian", () => {
  ...
  it("does not invent a vault when map has none", async () => {
    ...
    expect(
      findings.filter((f) =>
        ["obsidian.vault_missing", "obsidian.missing_vault_link", "obsidian.broken_vault_link"].includes(
          f.id,
        ),
      ),
    ).toEqual([]);
  ...
  it("flags missing vault links in instruction files when vault exists", async () => {
    ...
    const missingLink = findings.filter(
  ...
  it("does not flag when instruction or memoryPointers reference the vault", async () => {
    ...
    expect(
      findings.filter((f) => f.id === "obsidian.missing_vault_link"),
    ).toEqual([]);
  });
